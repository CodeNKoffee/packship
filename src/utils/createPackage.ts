...
export async function createPackage() {
  ...
  const nameStr: string = String(name), 
    descriptionStr: string = String(description), 
    languageChoiceStr: string = String(languageChoice);
  ...
  const projectTypeStr: string = String(projectType);
  ...
  const {
    ...
    includeReadme
  } = await getAdditionalChoices();
  const includeInternalDirBool = Boolean(includeInternalDir),
    useWebpackBool = Boolean(useWebpack),
    useEslintBool = Boolean(useEslint),
    usePostcssBool = Boolean(usePostcss),
    useNpmignoreBool = Boolean(useNpmignore),
    includeLicenseBool = Boolean(includeLicense),
    includeCodeOfConductBool = Boolean(includeCodeOfConduct),
    includeReadmeBool = Boolean(includeReadme);

  const licenseType = includeLicense ? await getLicenseType() : undefined;
  const licenseTypeStr: string = String(licenseType);

  // Building the package 
  let packageData: PackageData = buildPackageData(nameStr, descriptionStr, languageChoiceStr, licenseTypeStr);
